// 本文件实现的是基于Test-and-Set (TAS)的自旋锁
...
struct Tas_lock
{
    volatile int is_locked; // 0表示锁是自由的，1表示锁被占用
};

// 初始化锁，一开始未被占用
void init_lock(Tas_lock *lock)
{
    lock->is_locked = 0;
}

// 获取锁
void acquire_lock(Tas_lock *lock)
{
...
// 释放锁
void release_lock(Tas_lock *lock)
{
...
void histogram(struct img *input, int *hist_r, int *hist_g, int *hist_b)
{
    ...
    // 为rgb的每个取值分别设置自己的锁
    Tas_lock *locks_r = (Tas_lock *)malloc((maxrgb + 1) * sizeof(Tas_lock));
    Tas_lock *locks_g = (Tas_lock *)malloc((maxrgb + 1) * sizeof(Tas_lock));
    Tas_lock *locks_b = (Tas_lock *)malloc((maxrgb + 1) * sizeof(Tas_lock));
